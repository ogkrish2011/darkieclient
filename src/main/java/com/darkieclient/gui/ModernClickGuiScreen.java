...
public final class ModernClickGuiScreen extends GuiScreen {
    ...
    private static final int SURFACE_WINDOW_BORDER = 0x233554;
    private static final int SURFACE_WINDOW = 0x0A192F;
    private static final int SURFACE_WINDOW_HEADER = 0x112240;
    private static final int SURFACE_WINDOW_BODY = 0x0A192F;
    private static final int SURFACE_PANEL = 0x112240;
    private static final int SURFACE_PANEL_OUTLINE = 0x233554;
    private static final int SURFACE_ROW = 0x1D2D44;
    private static final int SURFACE_INPUT = 0x020C1B;
    private static final int SURFACE_CHIP = 0x233554;
    private static final int TEXT_PRIMARY = 0xFFCCD6F6;
    private static final int TEXT_SECONDARY = 0xFF8892B0;
    private static final int TEXT_MUTED = 0xFF495670;
    private static final int TEXT_DISABLED = 0xFF303C55;
    private static final int SNOWFLAKE_COLOR = 0x64FFDA;
    private static final int CONTROL_BACKGROUND = 0x020C1B;
    private static final int CONTROL_BORDER = 0x112240;
    private static final int CONTROL_BORDER_LIGHT = 0x233554;
    private static final int SWITCH_OFF_TRACK = 0x1D2D44;
    private static final int SWITCH_OFF_THUMB = 0x495670;
    private static final int SMALL_LABEL_COLOR = 0xFF8892B0;
    private static final int CHECKBOX_LABEL_COLOR = 0xFFCCD6F6;

    private static final int GRADIENT_START = 0x004E92;
    private static final int GRADIENT_END = 0x00D2FF;
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        GlStateManager.translate(layout.windowX + layout.getWidth() / 2.0F, layout.windowY + (layout.windowBottom - layout.windowY) / 2.0F, 0);
        ...
        if (drawSnowflakes) {
            ...
            drawSnowflakes(layout.windowBounds, 0.55F * openProgress);
            endScissor();
        }
        ...
        if (drawSnowflakes) {
            beginScissor(layout.windowBounds);
            drawSnowflakes(layout.windowBounds, 0.16F * openProgress);
            endScissor();
        }
    ...
    private void drawWindow(Layout layout, int accent) {
        ...
        // Header gradient
        drawGradientRect(layout.windowX + 1, layout.windowY + 1, layout.windowRight - 1, layout.windowY + HEADER_HEIGHT + 10, withAlpha(GRADIENT_START, 244), withAlpha(GRADIENT_END, 200));
    ...
    private void drawHeader(Layout layout, int mouseX, int mouseY, int accent) {
        this.fontRendererObj.drawStringWithShadow("Darkie Client", layout.windowX + 16, layout.windowY + 15, TEXT_PRIMARY);
        drawScaledText(DarkieClient.VERSION, layout.windowX + 16, layout.windowY + 28, TEXT_SECONDARY, SMALL_LABEL_SCALE);
        ...
        for (Category category : Category.values()) {
            ...
            // Hover scale effect
            float hoverScale = 1.0F + (animation * 0.05F);
            GlStateManager.pushMatrix();
            GlStateManager.translate(bounds.left + bounds.getWidth() / 2.0F, bounds.top + bounds.getHeight() / 2.0F, 0);
    ...
    private void drawModuleList(Layout layout, Category category, float scroll, int mouseX, int mouseY, int accent, int xOffset, boolean interactive, float alphaScale) {
        ...
        for (Module module : modules) {
            ...
            if (rowBounds.bottom >= layout.moduleScrollBounds.top && rowBounds.top <= layout.moduleScrollBounds.bottom) {
                ...
                // Hover scale effect
                float hoverScale = 1.0F + (selectionAnimation * 0.03F);
                GlStateManager.pushMatrix();
                GlStateManager.translate(rowBounds.left + rowBounds.getWidth() / 2.0F, rowBounds.top + rowBounds.getHeight() / 2.0F, 0);
                GlStateManager.scale(hoverScale, hoverScale, 1.0F);
                GlStateManager.translate(-(rowBounds.left + rowBounds.getWidth() / 2.0F), -(rowBounds.top + rowBounds.getHeight() / 2.0F), 0);
    ...
    private void drawHeaderToggle(Bounds bounds, boolean enabled, float progress, int accent, float alphaScale) {
        ...
        // Rounded track
        drawRoundedRect(bounds.left, trackTop, trackRight, trackTop + SWITCH_HEIGHT, SWITCH_HEIGHT / 2.0F, scaleAlpha(trackColor, alphaScale));
        drawRoundedOutline(bounds.left, trackTop, trackRight, trackTop + SWITCH_HEIGHT, SWITCH_HEIGHT / 2.0F, scaleAlpha(0xFF000000 | mixColor(CONTROL_BORDER, accent, progress * 0.7F), alphaScale));
        ...
        int thumbColor = 0xFF000000 | mixColor(SWITCH_OFF_THUMB, TEXT_PRIMARY, progress);
        
        // Rounded thumb
        drawRoundedRect(thumbLeft, trackTop + 1, thumbLeft + thumbSize, trackTop + 1 + thumbSize, thumbSize / 2.0F, scaleAlpha(thumbColor, alphaScale));
